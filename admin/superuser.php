...
superuser(){global$b;return$b;}function
...
set_password($Qh,$N,$V,$G){$_SESSION["pwds"][$Qh][$N][$V]=($_COOKIE["superuser_key"]&&is_string($G)?array(encrypt_string($G,$_COOKIE["superuser_key"])):$G);}function
get_password(){$J=get_session("pwds");if(is_array($J))$J=($_COOKIE["superuser_key"]?decrypt_string($J[0],$_COOKIE["superuser_key"]):false);return$J;}function
...
password_file($zb){$Fc=get_temp_dir()."/superuser.key";$J=@file_get_contents($Fc);if($J||!$zb)return$J;$Pc=@fopen($Fc,"w");if($Pc){$J=rand_string();fwrite($Pc,$J);fclose($Pc);}return$J;}function
rand_string(){return
md5(uniqid(mt_rand(),true));}function
select_value($X,$_,$n,$dh){global$b,$ba;if(is_array($X)){$J="";foreach($X
as$Dd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Dd):"")."<td>".select_value($W,$_,$n,$dh);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$n);if($_===null){if(is_mail($X))$_="mailto:$X";if($Gf=is_url($X))$_=(($Gf=="http"&&$ba)||preg_match('~WebKit~i',$_SERVER["HTTP_USER_AGENT"])?$X:"$Gf://www.superuser.org/redirect/?url=".urlencode($X));}$J=$b->editVal($X,$n);if($J!==null){if($J==="")$J="&nbsp;";elseif($dh!=""&&is_shortable($n)&&is_utf8($J))$J=shorten_utf8($J,max(0,+$dh));else$J=h($J);}return$b->selectVal($J,$_,$n,$X);}function
...
switch_lang(){global$ca,$Md;echo"<form action='' method='post'>\n<div id='lang'>",lang(19).": ".html_select("lang",$Md,$ca,"this.form.submit();")," <input type='submit' value='".lang(20)."' class='hidden'>\n","<input type='hidden' name='token' value='".get_token()."'>\n";echo"</div>\n</form>\n";}if(isset($_POST["lang"])&&verify_token()){cookie("superuser_lang",$_POST["lang"]);$_SESSION["lang"]=$_POST["lang"];$_SESSION["translations"]=array();redirect(remove_from_uri());}$ca="en";if(isset($Md[$_COOKIE["superuser_lang"]])){cookie("superuser_lang",$_COOKIE["superuser_lang"]);$ca=$_COOKIE["superuser_lang"];}elseif(isset($Md[$_SESSION["lang"]]))$ca=$_SESSION["lang"];else{$ua=array();preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~',str_replace("_","-",strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])),$be,PREG_SET_ORDER);foreach($be
...
false;}$_->query('PRAGMA encoding = "UTF-8"');$_->query('CREATE TABLE superuser (i)');$_->query('DROP TABLE superuser');return
true;}function
...
as$x=>$n)$o[$x]="  ".implode($n);$o=array_merge($o,array_filter($Jc));if(!queries("CREATE TABLE ".table($Q!=""?"superuser_$C":$C)." (\n".implode(",\n",$o)."\n)"))return
false;if($Q!=""){if($bf&&!queries("INSERT INTO ".table("superuser_$C")." (".implode(", ",$bf).") SELECT ".implode(", ",array_map('idf_escape',array_keys($bf)))." FROM ".table($Q)))return
...
false;queries("ALTER TABLE ".table("superuser_$C")." RENAME TO ".table($C));if(!alter_indexes($C,$v))return
false;foreach($vh
...
Min_DB;$Bb=$b->credentials();if($h->connect($Bb[0],$Bb[1],$Bb[2])){if($h->server_info>=9)$h->query("SET application_name = 'superuser'");return$h;}return$h->error;}function
get_databases(){return
...
connect($N,$V,$G){$this->_url="http:#";$J=$this->query('');if($J)$this->server_info=$J['version']['number'];return(bool)$J;}function
...
echo"<p><input type='submit' value='".lang(36)."'>\n",checkbox("auth[permanent]",1,$_COOKIE["superuser_permanent"],lang(37))."\n";}function
login($Xd,$G){return
true;}function
tableName($Og){return
h($Og["Name"]);}function
...
<title>',$kh,'</title>
<link rel="stylesheet" type="text/css" href="',h(preg_replace("~\\?.*~","",ME))."?file=default.css&amp;version=none",'">
<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=functions.js&amp;version=none",'"></script>
';if($b->head()){echo'<link rel="shortcut icon" type="image/x-icon" href="',h(preg_replace("~\\?.*~","",ME))."?file=&amp;version=none",'">
<link rel="#" href="',h(preg_replace("~\\?.*~","",ME))."?file=&amp;version=none",'">
';if(file_exists("superuser.css")){echo'<link rel="stylesheet" type="text/css" href="superuser.css">
';}}echo'
<body class="',lang(70),' nojs" onkeydown="bodyKeydown(event);" onclick="bodyClick(event);"',(isset($_COOKIE["superuser_version"])?"":" onload=\"verifyVersion('$ia');\""),'>
<script type="text/javascript">
...
long2str($W,true);}$h='';$bd=$_SESSION["token"];if(!$bd)$_SESSION["token"]=rand(1,1e6);$T=get_token();$qf=array();if($_COOKIE["superuser_permanent"]){foreach(explode(" ",$_COOKIE["superuser_permanent"])as$X){list($x)=explode(":",$X);$qf[$x]=$X;}}function
add_invalid_login(){global$b;$Fc=get_temp_dir()."/superuser.invalid";$Pc=@fopen($Fc,"r+");if(!$Pc){$Pc=@fopen($Fc,"w");if(!$Pc)return;}flock($Pc,LOCK_EX);$wd=unserialize(stream_get_contents($Pc));$eh=time();if($wd){foreach($wd
as$xd=>$X){if($X[0]<$eh)unset($wd[$xd]);}}$vd=&$wd[$b->bruteForceKey()];if(!$vd)$vd=array($eh+30*60,0);$vd[1]++;$tg=serialize($wd);rewind($Pc);fwrite($Pc,$tg);ftruncate($Pc,strlen($tg));flock($Pc,LOCK_UN);fclose($Pc);}$Ja=$_POST["auth"];if($Ja){$wd=unserialize(@file_get_contents(get_temp_dir()."/superuser.invalid"));$vd=$wd[$b->bruteForceKey()];$_e=($vd[1]>30?$vd[0]-time():0);if($_e>0)auth_error(lang(72,ceil($_e/60)));session_regenerate_id();$l=$Ja["driver"];$N=$Ja["server"];$V=$Ja["username"];$G=(string)$Ja["password"];$k=$Ja["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ja["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$Bf=$b->permanentLogin(true);$qf[$x]="$x:".base64_encode($Bf?encrypt_string($G,$Bf):"");cookie("superuser_permanent",implode(" ",$qf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($bd&&!verify_token()){page_header(lang(71),lang(73));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(74));}}elseif($qf&&!$_SESSION["pwds"]){session_regenerate_id();$Bf=$b->permanentLogin();foreach($qf
...
as$x=>$X){list($Qh,$N,$V,$k)=array_map('base64_decode',explode("-",$x));if($Qh==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$k==DB)unset($qf[$x]);}cookie("superuser_permanent",implode(" ",$qf));}function
auth_error($m){global$b,$bd;$wg=session_name();if(!$_COOKIE[$wg]&&$_GET[$wg]&&ini_bool("session.use_only_cookies"))$m=lang(75);elseif(isset($_GET["username"])){if(($_COOKIE[$wg]||$_GET[$wg])&&!$bd)$m=lang(76);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$m.='<br>'.lang(77,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("superuser_key",($_COOKIE["superuser_key"]?$_COOKIE["superuser_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(78),lang(79,implode(", ",$wf)),false);page_footer("auth");exit;}$h=connect();}$l=new
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="',lang(56),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>',lang(93),(support("comment")?"<td".($pb?"":" class='hidden'").">".lang(94):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=none' alt='+' title='".lang(95)."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($pb?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=none' alt='+' title='".lang(95)."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=none' alt='^' title='".lang(96)."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=none' alt='v' title='".lang(97)."'>&nbsp;":""),($Xe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=none' alt='x' title='".lang(98)."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
doc_link($of){global$w,$h;$Hh=array('sql'=>"#".substr($h->server_info,0,3)."/en/",'sqlite'=>"#",'pgsql'=>"#".substr($h->server_info,0,3)."/static/",'mssql'=>"#",'oracle'=>"#",);return($of[$w]?"<a href='$Hh[$w]$of[$w]' rel='noreferrer'><sup>?</sup></a>":"");}function
...
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".lang(123)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(126)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(62),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Rg=array();$Sg=array();$C="superuser_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$be,PREG_SET_ORDER);foreach($be
...
as$Xg)echo"\n<div class='references' title='".h($Yg)."' id='refd$Pd-".($r++)."' style='left: $Qd"."em; top: ".$Q["fields"][$Xg]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=none'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Qd)."em;'></div></div>";}}echo"\n</div>\n";}foreach($kg
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$m){$xb="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$x)$xb.="&$x=".urlencode($_POST[$x]);cookie("superuser_export",substr($xb,1));$S=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$yc=dump_headers((count($S)==1?key($S):DB),(DB==""||count($S)>1));$zd=preg_match('~sql~',$_POST["format"]);if($zd){echo"-- superuser $ia ".$Ub[DRIVER]." dump\n\n";if($w=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
...
lang(14),'">
';if(DB!="")echo"<input type='submit' name='drop' value='".lang(114)."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=none' alt='+' title='".lang(95)."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Fa=" AS\n$K[select]";$A=ME."table=".urlencode($C);$je=lang(185);if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Fa,$A,$je);else{$Zg=$C."_superuser_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Fa,"DROP VIEW ".table($C),"CREATE VIEW ".table($Zg).$Fa,"DROP VIEW ".table($Zg),($_POST["drop"]?substr(ME,0,-1):$A),lang(186),$je,lang(187),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$h->error;}page_header(($a!=""?lang(40):lang(188)),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
as$Ce=>$Tc){echo'<th>'.($Ce!="*.*"?"<input name='objects[$r]' value='".h($Ce)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server superuser"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(117),"Columns"=>lang(118),"Procedures"=>lang(226),)as$vb=>$Nb){foreach((array)$Df[$vb]as$Cf=>$ob){echo"<tr".odd()."><td".($Nb?">$Nb<td":" colspan='2'").' lang="en" title="'.h($ob).'">'.h($Cf);$r=0;foreach($Uc
as$Ce=>$Tc){$C="'grants[$r][".h(strtoupper($Cf))."]'";$Y=$Tc[strtoupper($Cf)];if($vb=="Server superuser"&&$Ce!=(isset($Uc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(227)."<option value='0'".($Y=="0"?" selected":"").">".lang(228)."</select>";else
...
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status1($a);$v=indexes($a);$o=fields($a);$Lc=column_foreign_keys($a);$Ee="";if($R["Oid"]){$Ee=($w=="sqlite"?"rowid":"oid");$v[]=array("type"=>"PRIMARY","columns"=>array($Ee));}parse_str($_COOKIE["superuser_import"],$xa);$cg=array();$f=array();$dh=null;foreach($o
...
as$x=>$X){if(in_array(idf_escape($x),$M))unset($Dh[$x]);}if($_POST["export"]){cookie("superuser_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Dh===array())$H="SELECT $Qc$Xh$Wc";else{$_h=array();foreach($_POST["check"]as$X)$_h[]="(SELECT".limit($Qc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$o).$Wc,1).")";$H=implode(" UNION ALL ",$_h);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Lc)){if($_POST["save"]||$_POST["delete"]){$I=true;$ya=0;$O=array();if(!$_POST["delete"]){foreach($f
